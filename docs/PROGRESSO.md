...
**Fase 2 (WhatsApp) adiada por decisão do usuário; próximo ciclo é uma Fase 3 recortada (sem campanhas por WhatsApp nem Meta Lead Ads).** Detalhe completo da decisão — o que exatamente entra/fica de fora e por quê — em `docs/decisoes/0004-fase2-adiada-fase3-sem-integracoes-externas.md`; `docs/PRD.md` §9 ganhou uma nota apontando pra lá, sem renumerar fases. Nenhum código foi escrito nesta rodada — puramente documentação, por escolha explícita do usuário (não construir `WhatsAppProvider`/schema agora, só quando a Fase 2 for retomada de verdade). Plano da Fase 3 recortada ainda não iniciado.
...
16. **Próximo ciclo real: Fase 3 recortada** (segmentos dinâmicos, campanhas por e-mail via `EmailProvider` mock, formulários, páginas de captura, webhook de entrada genérico, relatórios por origem — sem campanhas por WhatsApp nem Meta Lead Ads, ver ADR 0004). Ainda não planejado — passar por `spec-driven-development` antes de codar, como qualquer fase nova.
