...
> **Estado atual:** Fase 1C completa — 1A (fundação), 1B (auth/onboarding/convites), 1C-1 (Contatos), 1C-2 (Vencimentos) e 1C-3 (Importação) implementados; 1C-1/1C-2 testados no navegador, 1C-3 ainda pendente de teste no navegador. **1D-1 (layout)**, **1D-2 (Funis de venda)**, a **identidade visual (`ui-ux-pro-max`)** e **1D-3 (Tarefas)** implementados e validados no navegador. **1D-4 (Tela "Hoje")** implementado — substitui o placeholder de `/`: leads sem contato, follow-ups, negócios vencidos, vencimentos pendentes, aniversariantes, cards de resumo e checklist "Primeiros passos" — sem migration, lint/typecheck/test/build limpos, teste no navegador pendente. Falta só **1D-5** (PWA completo com push) pra fechar o 1D inteiro. **Pendência de lançamento:** `auth.email.enable_confirmations` precisa virar `true` antes de qualquer Supabase de staging/produção. Detalhes: `docs/PROGRESSO.md`.
