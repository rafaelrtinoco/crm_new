...
-- empresas
is((select count(*) from public.empresas where id = 'b0000000-0000-0000-0000-000000000001'), 0::bigint, 'não vê a empresa Beta');
is((select count(*) from public.empresas where id = 'a0000000-0000-0000-0000-000000000001'), 1::bigint, 'vê a própria empresa');
is((select count(*) from (update public.empresas set nome = nome where id = 'b0000000-0000-0000-0000-000000000001' returning id) x), 0::bigint, 'não altera a empresa Beta');

-- empresa_membros
is((select count(*) from public.empresa_membros where empresa_id = 'b0000000-0000-0000-0000-000000000001'), 0::bigint, 'não vê membros da empresa Beta');
is((select count(*) from (update public.empresa_membros set papel = papel where empresa_id = 'b0000000-0000-0000-0000-000000000001' returning id) x), 0::bigint, 'não altera membros da empresa Beta');

-- convites
is((select count(*) from public.convites where id = 'e0000000-0000-0000-0000-000000000002'), 0::bigint, 'não vê convite da empresa Beta');
is((select count(*) from (update public.convites set status = status where id = 'e0000000-0000-0000-0000-000000000002' returning id) x), 0::bigint, 'não altera convite da empresa Beta');

-- contatos
is((select count(*) from public.contatos where id = 'b0000000-0000-0000-0000-000000000301'), 0::bigint, 'não vê contato da empresa Beta');
is((select count(*) from public.contatos where empresa_id = 'a0000000-0000-0000-0000-000000000001'), 3::bigint, 'usuario vê os contatos sob sua responsabilidade na própria empresa');
is((select count(*) from (update public.contatos set nome = nome where id = 'b0000000-0000-0000-0000-000000000301' returning id) x), 0::bigint, 'não altera contato da empresa Beta');
is((select count(*) from (delete from public.contatos where id = 'b0000000-0000-0000-0000-000000000301' returning id) x), 0::bigint, 'não apaga contato da empresa Beta');
select throws_ok(
...
is((select count(*) from public.campos_personalizados where id = 'e0000000-0000-0000-0000-000000000012'), 0::bigint, 'não vê campo personalizado da empresa Beta');
is((select count(*) from (update public.campos_personalizados set ordem = ordem where id = 'e0000000-0000-0000-0000-000000000012' returning id) x), 0::bigint, 'não altera campo personalizado da empresa Beta');

-- tags
is((select count(*) from public.tags where id = 'b0000000-0000-0000-0000-000000000241'), 0::bigint, 'não vê tag da empresa Beta');
is((select count(*) from (update public.tags set cor = cor where id = 'b0000000-0000-0000-0000-000000000241' returning id) x), 0::bigint, 'não altera tag da empresa Beta');
select throws_ok(
...
-- contato_tags
is((select count(*) from public.contato_tags where id = 'e0000000-0000-0000-0000-000000000022'), 0::bigint, 'não vê vínculo de tag da empresa Beta');
is((select count(*) from (delete from public.contato_tags where id = 'e0000000-0000-0000-0000-000000000022' returning id) x), 0::bigint, 'não apaga vínculo de tag da empresa Beta');

-- vencimento_tipos
is((select count(*) from public.vencimento_tipos where id = 'b0000000-0000-0000-0000-000000000231'), 0::bigint, 'não vê tipo de vencimento da empresa Beta');
is((select count(*) from (update public.vencimento_tipos set ativo = ativo where id = 'b0000000-0000-0000-0000-000000000231' returning id) x), 0::bigint, 'não altera tipo de vencimento da empresa Beta');

-- vencimentos
is((select count(*) from public.vencimentos where descricao = 'Seguro auto — Onix'), 0::bigint, 'não vê vencimento da empresa Beta');
is(
  (select count(*) from (
...
is((select count(*) from public.funis where id = 'b0000000-0000-0000-0000-000000000201'), 0::bigint, 'não vê funil da empresa Beta');
is((select count(*) from (update public.funis set ativo = ativo where id = 'b0000000-0000-0000-0000-000000000201' returning id) x), 0::bigint, 'não altera funil da empresa Beta');
select throws_ok(
...
is((select count(*) from public.etapas where id = 'b0000000-0000-0000-0000-000000000211'), 0::bigint, 'não vê etapa da empresa Beta');
is((select count(*) from (update public.etapas set ordem = ordem where id = 'b0000000-0000-0000-0000-000000000211' returning id) x), 0::bigint, 'não altera etapa da empresa Beta');

is((select count(*) from public.motivos_perda where id = 'b0000000-0000-0000-0000-000000000221'), 0::bigint, 'não vê motivo de perda da empresa Beta');
is((select count(*) from (update public.motivos_perda set ativo = ativo where id = 'b0000000-0000-0000-0000-000000000221' returning id) x), 0::bigint, 'não altera motivo de perda da empresa Beta');

-- negocios
is((select count(*) from public.negocios where proximo_passo_acao = 'Enviar cotação por WhatsApp'), 0::bigint, 'não vê negócio da empresa Beta');
is(
  (select count(*) from (
...
-- atividades
is((select count(*) from public.atividades where id = 'e0000000-0000-0000-0000-000000000032'), 0::bigint, 'não vê atividade da empresa Beta');

-- tarefas
is((select count(*) from public.tarefas where titulo = 'Enviar cotação do seguro auto'), 0::bigint, 'não vê tarefa da empresa Beta');
is(
  (select count(*) from (
...
-- consentimentos
is((select count(*) from public.consentimentos where id = 'e0000000-0000-0000-0000-000000000042'), 0::bigint, 'não vê consentimento da empresa Beta');

-- audit_log
is((select count(*) from public.audit_log where id = 'e0000000-0000-0000-0000-000000000072'), 0::bigint, 'não vê audit_log da empresa Beta');
select throws_ok(
...
is((select count(*) from public.importacoes where id = 'e0000000-0000-0000-0000-000000000052'), 0::bigint, 'não vê importação da empresa Beta');
is((select count(*) from public.importacao_erros where id = 'e0000000-0000-0000-0000-000000000062'), 0::bigint, 'não vê erro de importação da empresa Beta');

-- nicho_templates: dado global, deve ser visível para qualquer autenticado,
-- mas não editável por quem não é service_role.
is((select count(*) from public.nicho_templates where nicho = 'corretora'), 1::bigint, 'vê o template global mesmo sem pertencer a nenhuma empresa dona dele');
select throws_ok(
  $$ update public.nicho_templates set nome_exibicao = 'Hackeado' where nicho = 'corretora' $$,
  '42501',
  null,
  'não consegue editar o template global de nicho'
);
